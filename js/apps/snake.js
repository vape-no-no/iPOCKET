...
function initSnake() {
  ...
  wrap.style.cssText = 'width:100%;height:100%;display:flex;flex-direction:column;overflow:hidden;background:#050508;position:relative;';
  content.appendChild(wrap);

  const hud = document.createElement('div');
  hud.id = 'snake-hud';
  hud.style.cssText = 'flex-shrink:0;background:var(--win-chrome);border-bottom:2px solid var(--win-chrome-dark);padding:4px 10px;font-family:var(--pixel-font);font-size:18px;color:var(--win-text);display:flex;justify-content:space-between;';
  hud.innerHTML = '<span>SCORE: 0</span><span id="snake-best">BEST: '+((window.POS?POS.getHighScore('snake'):0))+'</span>';
  wrap.appendChild(hud);

  const cvWrap = document.createElement('div');
  cvWrap.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#050508;';
  ...
  dpad.style.cssText = 'flex-shrink:0;display:grid;grid-template-columns:repeat(3,40px);grid-template-rows:repeat(2,40px);gap:2px;justify-content:center;align-items:center;padding:6px;background:var(--win-chrome);border-top:2px solid var(--win-chrome-light);';
  ...
  const CELL = 17;
  const avW = content.offsetWidth || 360;
  const avH = (content.offsetHeight || 500) - 48 - 130; // hud + dpad
  ...
  const reset = () => {
    ...
    score = 0; hud.querySelector('span').textContent = 'SCORE: 0';
  ...
  const step = () => {
    ...
    snake.unshift(head);
    if (head.x === food.x && head.y === food.y) { score++; hud.querySelector('span').textContent = `SCORE: ${score}`; placeFood(); }
    else snake.pop();
    draw();
  };
  ...
  const gameOver = () => {
    ...
    ov.style.cssText = 'position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;background:rgba(192,192,192,.94);z-index:10;';
