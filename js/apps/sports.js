...
function initSports() {
  ...
  root.style.cssText = 'width:100%;height:100%;display:flex;flex-direction:column;background:var(--win-chrome);overflow:hidden;position:relative;';
  content.appendChild(root);

  /* ── Header ── */
  const header = document.createElement('div');
  header.style.cssText = 'flex-shrink:0;padding:8px 16px 0;background:var(--win-chrome);';
  ...
  gridPanel.style.cssText = 'position:absolute;inset:0;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:12px 14px 40px;background:var(--win-chrome);';
  body.appendChild(gridPanel);

  /* ── Games panel ── */
  const gamesPanel = document.createElement('div');
  gamesPanel.style.cssText = 'position:absolute;inset:0;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:8px 14px 40px;background:var(--win-chrome);transform:translateX(100%);transition:transform .32s cubic-bezier(.34,1.56,.64,1);';
