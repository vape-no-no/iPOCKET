...
window.OS = {
  openApp(appId) {
    ...
    // If already open, focus it (and un-minimize it if it was hidden)
...
// Re-derive every open window's stacking position from its order in `openWindows`.
// Windows always occupy a small, bounded z-index band (300..300+N) that stays safely
// under the fixed chrome bands (notif 550 / start-menu 600 / switcher 700 / dialogs 800+),
// no matter how many times someone opens or refocuses windows in a session.
// Also mirrored onto a --win-z custom property: the liquid theme pins .win98-window's
// z-index with !important, which would otherwise silently swallow this entirely.
function restackWindows() {
  openWindows.forEach((w, i) => {
    const z = 300 + i;
    w.el.style.zIndex = z;
    w.el.style.setProperty('--win-z', z);
  });
}
...
function createWindow98(meta) {
  ...
  let savedPos = null; // { left, top, width, height }

  function enterFloatMode() {
    /* Convert from CSS-position (fixed inset) to explicit left/top/w/h */
    if (!el.style.left) {
  ...
  function clampWindow() {
    /* Keep window fully inside the viewport */
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    const minW = 280, minH = 180;
    let l = parseFloat(el.style.left)   || 0;
    let t = parseFloat(el.style.top)    || 0;
    let w = parseFloat(el.style.width)  || vw;
    let h = parseFloat(el.style.height) || vh;
  ...
  minBtn.addEventListener('click', () => {
    ...
    if (document.body.classList.contains('theme-modern') || document.body.classList.contains('theme-liquid')) {
      ...
      animateWindowToTaskbar(el, btn);
      setTimeout(() => { el.style.display = 'none'; updateTaskbar(); }, 80);
    }
  });
  ...
  maxBtn.addEventListener('click', () => {
    if (isMaximised) {
      ...
      if (savedPos) {
        ...
        el.style.width  = savedPos.width;
        el.style.height = savedPos.height;
        el.style.right  = 'auto';
        el.style.bottom = 'auto';
        el.style.position = 'fixed';
      } else {
        /* Reset to full-screen defaults */
  ...
  titlebar.addEventListener('pointerdown', e => {
    if (e.target.closest('.win-controls')) return; // don't drag via buttons
  ...
  titlebar.addEventListener('pointermove', e => {
    if (!drag) return;
    el.style.left = (drag.origL + e.clientX - drag.startX) + 'px';
    el.style.top  = (drag.origT + e.clientY - drag.startY) + 'px';
    clampWindow();
  });
  ...
  /* ── RESIZE via HANDLES ──────────────────────────── */
  el.querySelectorAll('.win-resize-handle').forEach(handle => {
    let rs = null;
    ...
    handle.addEventListener('pointermove', e => {
      ...
      let { origL: l, origT: t, origW: w, origH: h } = rs;

      if (rs.dir.includes('e')) w = Math.max(MIN_W, w + dx);
      if (rs.dir.includes('s')) h = Math.max(MIN_H, h + dy);
      if (rs.dir.includes('w')) { const nw = Math.max(MIN_W, w - dx); l += (w - nw); w = nw; }
      if (rs.dir.includes('n')) { const nh = Math.max(MIN_H, h - dy); t += (h - nh); h = nh; }
  ...
  el.addEventListener('pointerdown', (e) => {
    if (!e.target.closest('.win-controls') && !e.target.closest('.win-resize-handle')) {
